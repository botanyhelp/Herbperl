#!/usr/bin/perl
# lichensearchspecimen.cgi - wisflora specimen records listing
use strict;
use warnings;
use CGI;
use DBI;
use URI::URL;
my $title = "lichensearchspecimen.cgi";
my $cgi = new CGI();
print $cgi->header(); 
...
<table width="100%" border="0">
  <tr bgcolor="#EEEEEE">
    <td align="center" width="25%">
      <p><strong>Menu:</strong> <font size=2><a href="/wislichens/"><b>Lichen Home</b></a></font> </p></td>
    <td align="center" width="25%"> <font size="2"><a href="/wislichens/"><b></b></a></font></td>
    <td align="center" width="25%"> <font size="2"><a href="/wislichens/"><b></b></a></font> </td>
    <td align="center" width="25%"> <font size="2"><a href="/wislichens/"><b></b></a></font> </td>
  </tr>
</table>
...
{ # begin scope
	...
	if($typeSpecimenParam = $cgi->param("typeSpecimen")){
		...
		my $sthone = $dbhone->prepare ("SELECT specimenlichen.ACCESSION, specimenlichen.TAXCD, spdetail.Taxa FROM specimenlichen INNER JOIN spdetail ON specimenlichen.TAXCD = spdetail.Taxcd WHERE (spdetail.syncd='.' AND specimenlichen.TYPE <> ' ' AND specimenlichen.scan ='1') ORDER BY spdetail.Taxa, specimenlichen.ACCESSION");
		$sthone->execute();
		my @rows = ();
		push (@rows, $cgi->Tr (
	 	...
	 	while (my ($ACCESSION, $Taxcd, $Taxa) = $sthone->fetchrow_array ()){
			...
			push (@rows, $cgi->Tr (
		                $cgi->td({-bgcolor => $bgcolor}, $cgi->a({-href => "/cgi-bin/lichenspecimen.cgi?Accession=$ACCESSION"}, CGI::escapeHTML ($Taxa))),
		                $cgi->td({-bgcolor => $bgcolor}, $cgi->a({-href => "/cgi-bin/lichenspecimen.cgi?Accession=$ACCESSION"}, CGI::escapeHTML ($ACCESSION)))
		         ));
		#END while
		}
	...
	#Capture endangered queries second:
		...
		my $sthone = $dbhone->prepare ("SELECT specimenlichen.ACCESSION, specimenlichen.TAXCD, spdetail.Taxa FROM specimenlichen INNER JOIN spdetail ON specimenlichen.TAXCD=spdetail.Taxcd WHERE specimenlichen.Scan = '1' AND spdetail.Syncd='.' ORDER BY spdetail.Taxa, specimenlichen.ACCESSION");
		$sthone->execute();
		my @rows = ();
		push (@rows, $cgi->Tr (
		              $cgi->th ({-bgcolor => $bgcolor}, "Taxon"),
	...
	#Capture long-form searchSpecimen.html queries third:
		...
		my $sqlone = "SELECT * FROM specimenlichen, spdetail WHERE spdetail.syncd = '.' ";
		...
		if($PlaceParam = $cgi->param("Place")){ $gqs .= '&' ; $gqs .= "Place"; $gqs .= '='; $gqs .= $PlaceParam;
		       $PlaceParam .= '%'; $sqlone .= " AND specimenlichen.Place LIKE '$PlaceParam' ";
		}else{
		        $PlaceParam= "";
		}
		if($TownParam = $cgi->param("Town")){ $gqs .= '&' ; $gqs .= "Town"; $gqs .= '='; $gqs .= $TownParam;
		       $TownParam .= '%'; $sqlone .= " AND (specimenlichen.T1 LIKE '$TownParam' OR specimenlichen.T2 LIKE '$TownParam') ";
		}else{
		        $TownParam= "";
		}
		if($RangeParam = $cgi->param("Range")){ $gqs .= '&' ; $gqs .= "Range"; $gqs .= '='; $gqs .= $RangeParam;
		       $RangeParam .= '%'; $sqlone .= " AND (specimenlichen.R1 LIKE '$RangeParam' OR specimenlichen.R2 LIKE '$RangeParam') ";
		}else{
		        $RangeParam= "";
		}
		if($SectionParam = $cgi->param("Section")){ $gqs .= '&' ; $gqs .= "Section"; $gqs .= '='; $gqs .= $SectionParam;
		       $SectionParam .= '%'; $sqlone .= " AND (specimenlichen.S1 LIKE '$SectionParam' OR specimenlichen.S2 LIKE '$SectionParam') ";
		}else{
		        $SectionParam= "";
		}
		if($txtSearchAccessionParam = $cgi->param("txtSearchAccession")){ $gqs .= '&' ; $gqs .= "txtSearchAccession"; $gqs .= '='; $gqs .= $txtSearchAccessionParam;
		       $txtSearchAccessionParam .= '%'; $sqlone .= " AND specimenlichen.ACCESSION LIKE '$txtSearchAccessionParam' ";
		}else{
		        $txtSearchAccessionParam= "";
		}
		if($CountyParam = $cgi->param("County")){ $gqs .= '&' ; $gqs .= "County"; $gqs .= '='; $gqs .= $CountyParam;
		       $CountyParam .= '%'; $sqlone .= " AND specimenlichen.County LIKE '$CountyParam' ";
		}else{
		        $CountyParam= "";
		}
		if($txtSearchCountryParam = $cgi->param("txtSearchCountry")){ $gqs .= '&' ; $gqs .= "txtSearchCountry"; $gqs .= '='; $gqs .= $txtSearchCountryParam;
		       $txtSearchCountryParam .= '%'; $sqlone .= " AND specimenlichen.Country LIKE '$txtSearchCountryParam' ";
		}else{
		...
		if($txtSearchHabitatParam = $cgi->param("txtSearchHabitat")){ $gqs .= '&' ; $gqs .= "txtSearchHabitat"; $gqs .= '='; $gqs .= $txtSearchHabitatParam;
		       $txtSearchHabitatParam .= '%'; $sqlone .= " AND (specimenlichen.Habitat LIKE '$txtSearchHabitatParam' OR specimenlichen.Habitat_MISC LIKE '$txtSearchHabitatParam') ";
		}else{
		        $txtSearchHabitatParam= "";
		}
		if($CollectorParam = $cgi->param("Collector")){ $gqs .= '&' ; $gqs .= "Collector"; $gqs .= '='; $gqs .= $CollectorParam;
		       $CollectorParam .= '%'; $sqlone .= " AND specimenlichen.Coll1Name LIKE '$CollectorParam' ";
		}else{
		        $CollectorParam= "";
		}
		if($ColleventParam = $cgi->param("Collevent")){ $gqs .= '&' ; $gqs .= "Collevent"; $gqs .= '='; $gqs .= $ColleventParam;
		       $ColleventParam .= '%'; $sqlone .= " AND specimenlichen.Collevent LIKE '$ColleventParam' ";
		}else{
		        $ColleventParam= "";
		}
		if($chkSearchTypeParam = $cgi->param("chkSearchType")){ $gqs .= '&' ; $gqs .= "chkSearchType"; $gqs .= '='; $gqs .= $chkSearchTypeParam;
		       $chkSearchTypeParam .= '%'; $sqlone .= " AND specimenlichen.Type IS NOT NULL' ";
		}else{
		        $chkSearchTypeParam= "";
		}
		if($chkSearchScanParam = $cgi->param("chkSearchScan")){ $gqs .= '&' ; $gqs .= "chkSearchScan"; $gqs .= '='; $gqs .= $chkSearchScanParam;
		       $chkSearchScanParam .= '%'; $sqlone .= " AND specimenlichen.Scan IS NOT NULL";
		}else{
		        $chkSearchScanParam= "";
		}
		#   SORT START######################
		...
		#   SORT STOP   #############
		$sortdir = 'ASC';
		$sqlone .= " AND specimenlichen.TAXCD = spdetail.Taxcd ORDER BY $sortop $sortdir";
...
#This while loop does not have the CHANGETIME thing because it runs on MySQL 3.s23 and so doesn't have changeime
		while(my ($id, $ACCESSION, $TYPE, $COLLDATE, $FLOWER, $FRUIT, $STERILE, $OBJTYPE, $INST, $ANNCODE, $ANNDATE, $ANNSOURCE, $CITY, $SITENO, $CITYTYPE, $COLL2NAME, $COLL3NAME, $COLL1NAME, $COLLNO1, $COLLEVENT, $TAXCD, $CFS, $CFV, $CFVariety, $HABITAT_MISC, $HABITAT, $LONGX, $LAT, $ELEV, $LLGENER, $LONG2, $LAT2, $LTDEC, $LGDEC, $NOWLOC, $LOAN, $PAGES, $ORIGCD, $PUBCD, $LITCIT, $PUBDATE, $PUBDATEA, $VERPERS, $VERDATE, $EX, $ARTICLE, $PREC, $STATEL, $COUNTY, $COUNTRY, $T1, $R1, $S1, $NSEW_1, $TRSGENER, $T2, $R2, $S2, $NSEW_2, $PLACE, $scan, $MAPFILE, $username, $date_time, $DTRS, $ChemistryLN, $SubstrateLN, $ColorLN, $specimen_PKID, $Taxcd, $Syncd, $family_code, $genus, $species, $authority, $subsp, $variety, $forma, $subsp_auth, $var_auth, $forma_auth, $sub_family, $tribe, $common, $Wisc_found, $ssp, $var, $f, $hybrids, $status_code, $hide, $USDA, $COFC, $WETINDICAT, $FAM_NAME, $FAMILY, $GC, $FAMILY_COMMON, $SYNWisc_found, $SYNS_STATUS, $SYNV_STATUS, $SYNF_STATUS, $SYNHYBRIDS_STATUS, $SYNW_STATUS, $speciesweb_Taxcd, $Status, $Photo, $Photographer, $Thumbmaps, $Accgenus, $SORTOR, $Hand, $growth_habit_bck, $blooming_dt_bck, $origin_bck, $Thumbphoto, $date_time_TWO, $growth_habit, $blooming_dt, $origin, $Taxa, $growthform, $photobiont, $abundance, $Gallery) = $sthone->fetchrow_array()){
			...
			my $sthtwo = $dbhone->prepare ("SELECT Taxa FROM spdetail WHERE Taxcd = '$Taxcd' AND Syncd = '.'");
			$sthtwo->execute();
			while (my ($Taxa) = $sthtwo->fetchrow_array ()){
			...
			push (@rows, $cgi->Tr (
		                $cgi->td({-bgcolor => $bgcolor}, $cgi->a({-href => "/cgi-bin/lichenspecimen.cgi?Accession=$ACCESSION"}, CGI::escapeHTML ($ACCESSION))),
		                $cgi->td({-bgcolor => $bgcolor}, $cgi->a({-href => "/cgi-bin/lichendetail.cgi?SpCode=$TAXCD"}, CGI::escapeHTML($taxonName))),
	...
	#run long result sets.
		...
		my $total_recs = $dbhone->selectrow_array ("SELECT COUNT(*) FROM specimenlichen WHERE taxcd = '$SpCodeParam'");
		#METERING CODE
		#Feb0908...change for sortop ordering
		#ensure $sortop makes sense, or is ACCESSION
		#unless($sortop){$sortop = 'ACCESSION'}
		#my $stmt = sprintf ("SELECT * FROM specimenlichen WHERE taxcd = '$SpCodeParam' ORDER BY $sortop LIMIT %d,%d", $start-1, $per_page);
		my $stmt = sprintf ("SELECT *, LEFT(RIGHT(COLLDATE,12), 4) rcoll, RIGHT(LEFT(COLLDATE,2),1) lcoll FROM specimenlichen WHERE taxcd = '$SpCodeParam' ORDER BY $sortop LIMIT %d,%d", $start-1, $per_page);
		...
		# empty because the db won't return that many fields.
	 	while (my ($idd, $ACCESSION, $TYPE, $COLLDATE, $FLOWER, $FRUIT, $STERILE, $OBJTYPE, $INST, $ANNCODE, $ANNDATE, $ANNSOURCE, $CITY, $SITENO, $CITYTYPE, $COLL2NAME, $COLL3NAME, $COLL1NAME, $COLLNO1, $COLLEVENT, $TAXCD, $CFS, $CFV, $CFVariety, $HABITAT_MISC, $HABITAT, $LONGX, $LAT, $ELEV, $LLGENER, $LONG2, $LAT2, $LTDEC, $LGDEC, $NOWLOC, $LOAN, $PAGES, $ORIGCD, $PUBCD, $LITCIT, $PUBDATE, $PUBDATEA, $VERPERS, $VERDATE, $EX, $ARTICLE, $PREC, $STATEL, $COUNTY, $COUNTRY, $T1, $R1, $S1, $NSEW_1, $TRSGENER, $T2, $R2, $S2, $NSEW_2, $PLACE, $scan, $MAPFILE, $username, $date_time, $DTRS,  $ChemistryLN, $SubstrateLN, $ColorLN, $id) = $sthone->fetchrow_array ()){
			#$sthtwo is run so that we can obtain the "Taxa" name from spdetail for this particular specimen:
			#notice that this next query has strong potential for being rerun over-and-over-and-over again, room for improvement here:
			my $sthtwo = $dbhone->prepare ("SELECT Taxa, FAMILY FROM spdetail WHERE Taxcd = '$SpCodeParam' AND Syncd = '.'");
			$sthtwo->execute();
			...
			push (@rows, $cgi->Tr (
		                $cgi->td({-bgcolor => $bgcolor}, $cgi->a({-href => "/cgi-bin/lichenspecimen.cgi?Accession=$ACCESSION"}, CGI::escapeHTML ($ACCESSION))),
		                $cgi->td({-bgcolor => $bgcolor}, $cgi->a({-href => "/cgi-bin/lichendetail.cgi?SpCode=$SpCodeParam"}, CGI::escapeHTML($taxonName))),
		                $cgi->td({-bgcolor => $bgcolor}, CGI::escapeHTML ($COLLDATE)),
		                $cgi->td({-bgcolor => $bgcolor}, CGI::escapeHTML ($COLL1NAME)),
		                $cgi->td({-bgcolor => $bgcolor}, CGI::escapeHTML ($COLLNO1)),
		                $cgi->td({-bgcolor => $bgcolor}, CGI::escapeHTML ($COUNTY))
