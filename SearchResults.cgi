#!/usr/bin/perl
# SearchResults.cgi -wisflora Results page for species searching
use strict;
use warnings;
use CGI;
use DBI;
...
<table width="100%" border="0">
  <tr bgcolor="#EEEEEE">
    <td align="center" width="25%">
      <p><strong>Menu:</strong> <font size=2><a href="/herbarium/"><b>Herbarium
 Home</b></a></font> </p></td>
    <td align="center" width="25%"> <font size="2"><a href="/wisflora/"><b>
WISFLORA: Vascular Plant Species</b></a></font></td>
    <td align="center" width="25%"> <font size="2"><a href="/herb/search.html">
<b>Vascular Plant Taxon Search</b></a></font> </td>
    <td align="center" width="25%"> <font size="2"><a 
href="/herb/specimenSearch.html"><b>Search Specimen Database</b></a></font> 
</td>
  </tr>
</table>
HERBHEADER
		...
		if(exists($families->{$fam}->{$genusParam}) and $genusFirst == 0) {
			...
			foreach $t (sort keys %{$families->{$fam}->{$genusParam}} ){
				if(Dumper($families->{$fam}->{$genusParam}->{$t})=~/'\.',/ ){
					print("<br>&nbsp;&nbsp;&nbsp;&nbsp;");
					print $cgi->a({-href=>"/cgi-bin/detail.cgi?SpCode=".
...
<table width="100%" border="0">
  <tr bgcolor="#EEEEEE">
    <td align="center" width="25%">
      <p><strong>Menu:</strong> <font size=2><a href="/herbarium/"><b>
Herbarium Home</b></a></font> </p></td>
    <td align="center" width="25%"> <font size="2"><a href="/wisflora/"><b>
WISFLORA: Vascular Plant Species</b></a></font></td>
    <td align="center" width="25%"> <font size="2"><a href="/herb/search.html">
<b>Vascular Plant Taxon Search</b></a></font> </td>
    <td align="center" width="25%"> <font size="2"><a href="/herb/specimenSearch.html">
<b>Search Specimen Database</b></a></font> </td>
  </tr>
</table>
HERBFOOTER
print $cgi->end_html();
...
sub getTaxonNm
{
	my ($taxcd, $syncd) = @_;
	my $taxonName = "";
	my $dsntwo = "DBI:mysql:host=localhost;database=herbfortynine";
	#following line,dbhtwo, removed because it causes a connection that 
	# we do not use.  We 
	# can/and-have-been using the dbh DBhandle all along, and this 
	# connection, dbhtwo, is
	# established, not used, and "implicitly closed" with exit().  Since 
	# we do not call 
	# this subroutine, getTaxonNm, ever, until after the dbh connection 
	# has been 
	# established, which means getTaxonNm will never be called with a dead 
	# DBHandle.  
	#my $dbhtwo = DBI->connect($dsntwo, "apache", "apachePASS") or die 
	# "cantconnect $!";
	#this query takes 0.00 sec. from mysql monitor.fast apparently
	my $sth = $dbh->prepare ("SELECT * FROM spdetail WHERE Taxcd = '$taxcd' 
AND Syncd = '$syncd' ORDER BY FAMILY, Taxcd, Syncd");
	$sth->execute();
 	while (my ($Taxcd, $Syncd, $family_code, $genus, $species, $authority, 
$subsp, $variety, $forma, $subsp_auth, $var_auth, $forma_auth, $sub_family, 
$tribe, $common, $Wisc_found, $ssp, $var, $f, $hybrids, $status_code, $hide, 
$USDA, $COFC, $WETINDICAT, $FAM_NAME, $FAMILY, $GC, $FAMILY_COMMON, 
$SYNWisc_found, $SYNS_STATUS, $SYNV_STATUS, $SYNF_STATUS, $SYNHYBRIDS_STATUS, 
$SYNW_STATUS, $speciesweb_Taxcd, $Status, $Photo, $Photographer, $Thumbmaps, 
$Accgenus, $SORTOR, $Hand, $growth_habit_bck, $blooming_dt_bck, $origin_bck, 
$Thumbphoto, $date_time, $growth_habit, $blooming_dt, $origin, $Taxa, 
$spdetail_id ) = $sth->fetchrow_array ()){
		...
		if($strcd=~/x/){
			...
			#my $x = $status_code;
			#not sure why we repeat forma concatenation...
			# apparently if there is a forma, then it won't we 
