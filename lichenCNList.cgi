#!/usr/bin/perl
# lichenCNList.cgi -lichen Common Name listing 
...
my $title = "lichenCNList.cgi";
my $cgi = new CGI();
print $cgi->header(); 
print $cgi->start_html(-title => $title, -bgcolor => "white");
my $dsn = "DBI:mysql:host=localhost;database=lichenfive";
...
my $sth = $dbh->prepare ("SELECT DISTINCT Common FROM spdetail WHERE Common IS NOT NULL AND Wisc_Found = 'W' AND Common NOT LIKE '(%' ORDER BY Common");
$sth->execute ();
