#!/usr/bin/perl
# excel_maker_custom.cgi - generate HTML tables
...
use IO::Socket;
use DBI;
my $title = "excel_maker_custom.cgi";
my $cgi= CGI->new();
print $cgi->header();
...
my $sth = $dbh->prepare("SELECT * FROM specimen WHERE taxcd = '$SpCodeParam'");
$sth->execute();
...
my $sheet1 = $workbook->add_worksheet("xinputspecimen");
