#!/usr/bin/perl
# lichenGenusList.cgi -wislichen Genus listing 
...
#my $sth = $dbh->prepare ("SELECT DISTINCT Genus FROM spdetail WHERE syncd = '.' AND WiscFound = 'W' ORDER BY Genus");
$sth->execute ();
...
#add an empty table row if we are at a new letter:
   if($al=~/[A-Z]/){ 
	...
	$cgi->td({-bgcolor => "white", -width => 95 }, "&nbsp;").$cgi->td({-bgcolor => "white"}, "&nbsp;")
        ...
        push (@rows, $cgi->Tr (
        $cgi->td({-bgcolor => "silver", -width => 95 }, "<font face=\"Times New Roman\"><font color=\"#800000\"><font size=+1>$al</font></font></font>").$cgi->td({-bgcolor => "white"}, $cgi->a({href=>"/cgi-bin/lichenSearchResults.cgi?Genus=$Genus"}, "$Genus").$anchor)
        ));
        $al = "";
}
