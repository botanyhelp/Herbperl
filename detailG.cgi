...
while (my ($LTDECgmap, $LGDECgmap) = $sthgmap->fetchrow_array ()){
	if($LTDECgmap==0) {
		$i++;
		next;
	}
	$gmapString .= "latlng = new GLatLng($LTDECgmap, $LGDECgmap);\n";
	$gmapString .= "map.addOverlay(new GMarker(latlng));\n";
	$i++;
}
...
#CENTER OF WI: 44.9826 Long: -89.5605
print("map.setCenter(new GLatLng(44.5826, -89.5605), 6);\n");
...
if($gmapString =~/...../) {
	print("<BR>Specimens of this species were collected in these locations:<BR>\n");
	print("<div id=\"map\" style=\"width: 400px; height: 400px\"></div>\n");
}
